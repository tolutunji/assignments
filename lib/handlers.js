...
handlers.listMenu.post = function(data, callback) {
    const protocol = typeof(data.payload.protocol) === 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1 ? data.payload.protocol : false;
    const url = typeof(data.payload.url) === 'string' && data.payload.url.trim().length > 0 ? data.payload.url.trim() : false;
    const method = typeof(data.payload.method) === 'string' && ['post', 'get', 'put', 'delete'].indexOf(data.payload.method) > -1 ? data.payload.method : false;
    const successCodes = typeof(data.payload.successCodes) === 'object' && data.payload.successCodes instanceof Array && data.payload.successCodes.length > 0 ? data.payload.successCodes : false;
    const timeOutSeconds = typeof(data.payload.timeOutSeconds) === 'number' && data.payload.timeOutSeconds % 1 === 0 && data.payload.timeOutSeconds.length >= 1 && data.payload.timeOutSeconds.length <= 5 ? data.payload.timeOutSeconds : false;
        ...
        _data.read('tokens', token, function(err, tokenData) {
            if(!err, tokenData) {
                ...
                _data.read('users', userEmail, function(err, userData) {
                    if(!err, userData) {
                        const userMenuList = typeof(userData.menuList) === 'object' && userData.menuList instanceof Array ? userData.menuList : [];

                        if(userMenuList.length < config.maxMenuList) {
                            const menuListId = helpers.createRandomString(16);
                            ...
                            _data.create('menu', menuListId, menuListObject, function(err) {
                                if(!err) {
                                    userData.menuList = userMenuList;
                                    userData.menuList.push(menuListId);

                                    _data.update('users', userEmail, userData, function(err) {
                                        if(!err) {
                                            callback(200, menuListObject)
                                        } else {
                                            callback(500, {'Error' : 'Could the update the user with the new menu'})
                                        }
                                    })
                                } else {
                                    callback(500, {'Error' : 'Could not create the new menuList'})
                                }
                            })
                            ...
                            callback(400, {'error' : 'The user already has the maximum number of checks ('+config.maxMenuList+').'})
                        }
                    } else {
                        callback (403)
                    }
